...
ranked_dates AS (
    -- 2) Rank best and worst days by total sales
    SELECT
        market_date,
        total_sales,
...
-- 3) Pick highest and lowest days and UNION them
SELECT 
    market_date,
    total_sales,
...
SELECT
    v.vendor_name,
    p.product_name,
    SUM(5 * t.original_price) AS total_revenue_per_product
FROM (
    -- x*y rows: each distinct vendor–product paired with every customer
    SELECT DISTINCT
        vi.vendor_id,
        vi.product_id,
        vi.original_price,
...
JOIN vendor  AS v ON v.vendor_id  = t.vendor_id
